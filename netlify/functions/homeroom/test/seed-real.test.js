...
await getDb();
const count = async (table) => (await (await getDb()).prepare(`SELECT COUNT(*) AS n FROM ${table}`).get()).n;

test('it loads every researched data set', async () => {
  await seedReal();
  assert.equal(await count('hr_deals'), PERKS.length);
  assert.equal(await count('hr_funders'), CAPITAL_MAP.length);
  assert.equal(await count('hr_atlas'), ATLAS_LABS.length);
  assert.equal(await count('hr_mentors'), NETWORK_MENTORS.length);
  assert.equal(await count('hr_modules'), LIBRARY_MODULES.length);
});

test('and creates nothing invented', async () => {
  ...
  for (const table of ['hr_orgs', 'hr_jobs', 'hr_events',
    ...
    assert.equal(await count(table), 0, `${table} should be empty`);
  }
});
...
test('no funder ships with a rating', async () => {
  assert.equal(await count('hr_funder_reviews'), 0);
...
test('re-running refreshes rather than duplicating', async () => {
  const before = { deals: await count('hr_deals'), funders: await count('hr_funders'), labs: await count('hr_atlas') };
  await seedReal();
  await seedReal();
  assert.deepEqual(
    { deals: await count('hr_deals'), funders: await count('hr_funders'), labs: await count('hr_atlas') },
    before,
  );
});
