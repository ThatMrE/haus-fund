...
const ROUTES = [
  ...
  ['GET', '/homeroom/mentors/requests', async (ctx) => await render(ctx, views.myRequestsPage(ctx, {
    requests: await Promise.all((await desk.requestsFor(ctx.user.id))
 ...
 */
export async function homeroomRoute(method, pathname) {
