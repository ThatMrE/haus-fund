...
async function uniqueSlug(table, text, fallback) {
  ...
  let candidate = base;
  let n = 2;
  while (await db.get(`SELECT 1 FROM ${table} WHERE slug = ?`, candidate)) {
    candidate = `${base}-${n++}`;
...
export async function createUser({ id, email, passwordHash, isAdmin = false }) {
  ...
  await sql.run('INSERT INTO users (id, email, password_hash, karma, created_at, is_admin) VALUES (?, ?, ?, 1, ?, ?)', id, email ? String(email).trim().toLowerCase() : null, passwordHash, now, isAdmin ? 1 : 0);
  return await getUser(id);
}

export async function getUser(id) {
  if (!id) return null;
  return await sql.get('SELECT * FROM users WHERE lower(id) = lower(?)', id) ?? null;
}

export async function getUserByEmail(email) {
  if (!email) return null;
  return await sql.get('SELECT * FROM users WHERE email = ?', String(email).trim().toLowerCase()) ?? null;
}

export async function setPassword(userId, passwordHash) {
  await sql.run('UPDATE users SET password_hash = ? WHERE id = ?', passwordHash, userId);
}

export async function userCount() {
  return (await sql.get('SELECT COUNT(*) AS n FROM users')).n;
}
...
export async function getUserBySupabaseId(supabaseId) {
  ...
  return await sql.get('SELECT * FROM users WHERE supabase_id = ?', String(supabaseId)) ?? null;
}

export async function linkSupabaseId(userId, supabaseId) {
  await sql.run('UPDATE users SET supabase_id = ? WHERE id = ?', String(supabaseId || ''), userId);
}

export async function setUserEmail(userId, email) {
  await sql.run('UPDATE users SET email = ? WHERE id = ?', email ? String(email).trim().toLowerCase() : null, userId);
}
...
export async function ensureMember(userId, defaults = {}) {
  ...
  const now = nowSeconds();
  await sql.run(`INSERT INTO hr_members (user_id, name, headline, org, role, cohort, location, bio,
...
export async function getMember(userId) {
  ...
  const row = await sql.get(`SELECT ${MEMBER_COLUMNS} FROM hr_members m
       JOIN users u ON u.id = m.user_id
       WHERE lower(m.user_id) = lower(?)`, userId);
  if (!row) return null;
...
export async function memberExpertise(userId) {
  return (await sql.all('SELECT tag FROM hr_expertise WHERE user_id = ? ORDER BY tag', userId))
    .map((r) => r.tag);
}
...
export async function updateMember(userId, patch) {
  ...
  if (sets.length) {
    ...
    await sql.run(`UPDATE hr_members SET ${sets.join(', ')} WHERE user_id = ?`, ...values);
  }
...
export async function setExpertise(userId, tags) {
  await sql.tx(async (db) => {
    await db.run('DELETE FROM hr_expertise WHERE user_id = ?', userId);
    for (const tag of tags.slice(0, 12)) {
      if (tag) await db.run('INSERT INTO hr_expertise (user_id, tag) VALUES (?, ?) ON CONFLICT DO NOTHING', userId, tag);
    }
  });
}

export async function touchMember(userId) {
  await sql.run('UPDATE hr_members SET last_seen_at = ? WHERE user_id = ?', nowSeconds(), userId);
}
...
export async function searchMembers({
  ...
  const rows = await sql.all(`SELECT ${MEMBER_COLUMNS} FROM hr_members m
       JOIN users u ON u.id = m.user_id
       WHERE ${clause}
       ORDER BY u.karma DESC, m.joined_at ASC
       LIMIT ? OFFSET ?`, ...params, limit, offset);
  for (const row of rows) row.expertise = await memberExpertise(row.user_id);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_members m JOIN users u ON u.id = m.user_id WHERE ${clause}`, ...params);
  return { members: rows, total };
}

export async function expertiseCloud(limit = 40) {
  return await sql.all(`SELECT tag, COUNT(*) AS count FROM hr_expertise
       GROUP BY tag ORDER BY count DESC, tag ASC LIMIT ?`, limit);
}

export async function cohorts() {
  return await sql.all(`SELECT cohort, COUNT(*) AS count FROM hr_members
...
export async function createOrg({ name, tagline = '', description = '', kind = 'startup', stage = 'idea',
  ...
  return await sql.tx(async (db) => {
    ...
    const info = await db.run(`INSERT INTO hr_orgs (slug, name, tagline, description, kind, stage, location, website,
                              cohort, founded, headcount, tags, created_by, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, name, tagline, description, kind, stage, location, website, cohort,
        founded ?? null, headcount ?? null, tags, createdBy, now, now);
    const id = info.id;
    await db.run('INSERT INTO hr_org_members (org_id, user_id, role, admin, joined_at) VALUES (?, ?, ?, 1, ?)', id, createdBy, 'founder', now);
    return id;
  });
}
...
export async function getOrg(idOrSlug) {
  ...
  const row = /^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_orgs WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_orgs WHERE slug = ?', String(idOrSlug));
  return row ?? null;
}
...
export async function updateOrg(id, patch) {
  ...
  await sql.run(`UPDATE hr_orgs SET ${sets.join(', ')} WHERE id = ?`, ...values);
  return await getOrg(id);
}

export async function orgTeam(orgId) {
  return await sql.all(`SELECT om.role, om.admin, om.joined_at, m.*, u.karma
       FROM hr_org_members om
       JOIN users u ON u.id = om.user_id
       LEFT JOIN hr_members m ON m.user_id = om.user_id
       WHERE om.org_id = ? ORDER BY om.admin DESC, om.joined_at ASC`, orgId);
}

export async function joinOrg(orgId, userId, role = '') {
  await sql.run('INSERT INTO hr_org_members (org_id, user_id, role, admin, joined_at) VALUES (?, ?, ?, 0, ?) ON CONFLICT DO NOTHING', orgId, userId, role, nowSeconds());
}

export async function leaveOrg(orgId, userId) {
  await sql.run('DELETE FROM hr_org_members WHERE org_id = ? AND user_id = ?', orgId, userId);
}

export async function isOrgAdmin(orgId, userId) {
  if (!userId) return false;
  const row = await sql.get('SELECT admin FROM hr_org_members WHERE org_id = ? AND user_id = ?', orgId, userId);
  return !!row?.admin;
}

export async function isOrgMember(orgId, userId) {
  if (!userId) return false;
  return !!await sql.get('SELECT 1 FROM hr_org_members WHERE org_id = ? AND user_id = ?', orgId, userId);
}

export async function userOrgs(userId) {
  return await sql.all(`SELECT o.*, om.role, om.admin FROM hr_org_members om
       JOIN hr_orgs o ON o.id = om.org_id
       WHERE om.user_id = ? ORDER BY om.joined_at DESC`, userId);
}
...
export async function searchOrgs({ q = '', kind = '', stage = '', tag = '', limit = PAGE_SIZE, offset = 0 } = {}) {
  ...
  const clause = where.join(' AND ');
  const orgs = await sql.all(`SELECT o.*, (SELECT COUNT(*) FROM hr_org_members om WHERE om.org_id = o.id) AS team_count
       FROM hr_orgs o WHERE ${clause} ORDER BY o.created_at DESC LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_orgs WHERE ${clause}`, ...params);
  return { orgs, total };
}
...
export async function createUpdate({ orgId, authorId, period = '', body, asks = '', metrics = '' }) {
  const info = await sql.run(`INSERT INTO hr_updates (org_id, author_id, period, body, asks, metrics, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?)`, orgId, authorId, period, body, asks, metrics, nowSeconds());
  return info.id;
}

export async function orgUpdates(orgId, { limit = 10, offset = 0 } = {}) {
  return await sql.all('SELECT * FROM hr_updates WHERE org_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?', orgId, limit, offset);
}

export async function recentUpdates(limit = 8) {
  return await sql.all(`SELECT up.*, o.name AS org_name, o.slug AS org_slug FROM hr_updates up
       JOIN hr_orgs o ON o.id = up.org_id ORDER BY up.created_at DESC LIMIT ?`, limit);
}
...
export async function createDeal({ vendor, title, category = 'other', summary = '', details = '',
  ...
  const info = await sql.run(`INSERT INTO hr_deals (slug, vendor, title, category, summary, details, worth, code, url,
                             expires_at, posted_by, created_at, access, requirement, checked)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, vendor, title, category, summary, details, worth, code, url,
...
export async function setDealCode(dealId, code) {
  await sql.run('UPDATE hr_deals SET code = ? WHERE id = ?', String(code || ''), dealId);
...
export async function getDeal(idOrSlug) {
  ...
  return (/^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_deals WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_deals WHERE slug = ?', String(idOrSlug))) ?? null;
}
...
export async function listDeals({ category = '', q = '', limit = 100, offset = 0 } = {}) {
  ...
  const clause = where.join(' AND ');
  const deals = await sql.all(`SELECT d.*, (SELECT COUNT(*) FROM hr_deal_claims c WHERE c.deal_id = d.id) AS claim_count
       FROM hr_deals d WHERE ${clause} ORDER BY d.created_at DESC LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_deals WHERE ${clause}`, ...params);
  return { deals, total };
}

export async function claimDeal(dealId, userId) {
  await sql.run('INSERT INTO hr_deal_claims (deal_id, user_id, created_at) VALUES (?, ?, ?) ON CONFLICT DO NOTHING', dealId, userId, nowSeconds());
  return await getDeal(dealId);
}

export async function dealClaimCount(dealId) {
  return (await sql.get('SELECT COUNT(*) AS n FROM hr_deal_claims WHERE deal_id = ?', dealId)).n;
}

export async function hasClaimed(dealId, userId) {
  if (!userId) return false;
  return !!await sql.get('SELECT 1 FROM hr_deal_claims WHERE deal_id = ? AND user_id = ?', dealId, userId);
}

export async function myClaims(userId) {
  return await sql.all(`SELECT d.*, c.created_at AS claimed_at FROM hr_deal_claims c
       JOIN hr_deals d ON d.id = c.deal_id WHERE c.user_id = ? ORDER BY c.created_at DESC`, userId);
}
...
export async function createFunder({ name, kind = 'vc', focus = '', stages = '', checkSize = '',
  ...
  const info = await sql.run(`INSERT INTO hr_funders (slug, name, kind, focus, stages, check_size, location, website, description, dilutive, added_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, name, kind, focus, stages, checkSize, location, website, description, int(dilutive), addedBy, nowSeconds());
  return info.id;
}
...
export async function getFunder(idOrSlug) {
  const db = sql;
  const row = /^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_funders WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_funders WHERE slug = ?', String(idOrSlug));
  if (!row) return null;
...
export async function funderRatings(funderId) {
  const row = await sql.get(`SELECT COUNT(*) AS review_count, AVG(rating) AS avg_rating,
       ...
       FROM hr_funder_reviews WHERE funder_id = ?`, funderId);
...
export async function listFunders({ q = '', kind = '', stage = '', minRating = 0, sort = 'rating',
  ...
  const funders = await sql.all(`SELECT * FROM (${scored}) WHERE COALESCE(avg_rating, 0) >= ? ORDER BY ${order} LIMIT ? OFFSET ?`, ...params, minRating, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM (${scored}) WHERE COALESCE(avg_rating, 0) >= ?`, ...params, minRating);
...
export async function upsertReview({ funderId, userId, rating, speed = null, valueAdd = null,
  ...
  await sql.run(`INSERT INTO hr_funder_reviews
       ...
       ON CONFLICT (funder_id, user_id) DO UPDATE SET
         ...
         created_at = excluded.created_at`, funderId, userId, rating, speed, valueAdd, founderFriendly, terms,
...
export async function funderReviews(funderId, { sort = 'helpful' } = {}) {
  ...
  return await sql.all(`SELECT r.*,
       ...
       FROM hr_funder_reviews r WHERE r.funder_id = ? ORDER BY ${order}`, funderId);
}

export async function myReview(funderId, userId) {
  if (!userId) return null;
  return await sql.get('SELECT * FROM hr_funder_reviews WHERE funder_id = ? AND user_id = ?', funderId, userId) ?? null;
}
...
export async function upsertPipeline({ userId, funderId, orgId = null, status = 'researching', amount = '', notes = '' }) {
  const now = nowSeconds();
  await sql.run(`INSERT INTO hr_pipeline (user_id, funder_id, org_id, status, amount, notes, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)
       ON CONFLICT (user_id, funder_id) DO UPDATE SET
         org_id = excluded.org_id, status = excluded.status, amount = excluded.amount,
         notes = excluded.notes, updated_at = excluded.updated_at`, userId, funderId, orgId, status, amount, notes, now, now);
}

export async function removePipeline(userId, funderId) {
  await sql.run('DELETE FROM hr_pipeline WHERE user_id = ? AND funder_id = ?', userId, funderId);
}

export async function pipeline(userId) {
  return await sql.all(`SELECT pl.*, f.name AS funder_name, f.slug AS funder_slug, f.kind AS funder_kind,
              (SELECT ROUND(AVG(rating), 1) FROM hr_funder_reviews r WHERE r.funder_id = f.id) AS avg_rating
       FROM hr_pipeline pl JOIN hr_funders f ON f.id = pl.funder_id
       WHERE pl.user_id = ? ORDER BY pl.updated_at DESC`, userId);
}

export async function pipelineEntry(userId, funderId) {
  if (!userId) return null;
  return await sql.get('SELECT * FROM hr_pipeline WHERE user_id = ? AND funder_id = ?', userId, funderId) ?? null;
}
...
export async function createSlot({ hostId, title, description = '', format = 'one-on-one',
  ...
  const info = await sql.run(`INSERT INTO hr_slots (host_id, title, description, format, starts_at, minutes, capacity,
                             place, topics, created_at, mentor_id, url)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, hostId, title, description, format, startsAt, minutes, Math.max(1, capacity),
      place, topics, nowSeconds(), mentorId, url);
  return info.id;
}

export async function getSlot(id) {
  const row = await sql.get(`SELECT s.*, n.name AS mentor_name, n.slug AS mentor_slug, n.org AS mentor_org,
              n.role AS mentor_role, n.track AS mentor_track, n.vetted AS mentor_vetted
       FROM hr_slots s LEFT JOIN hr_mentors n ON n.id = s.mentor_id WHERE s.id = ?`, Number(id));
  if (!row) return null;
  row.booked = (await sql.get('SELECT COUNT(*) AS n FROM hr_bookings WHERE slot_id = ?', row.id)).n;
  return row;
}
...
export async function listSlots({ upcoming = true, hostId = '', mentorId = 0, track = '', limit = 60 } = {}) {
  ...
  return await sql.all(`SELECT s.*, (SELECT COUNT(*) FROM hr_bookings b WHERE b.slot_id = s.id) AS booked,
       ...
       ORDER BY s.starts_at ${upcoming ? 'ASC' : 'DESC'} LIMIT ?`, ...params, limit);
}

export async function bookSlot(slotId, userId, question = '') {
  return await sql.tx(async (db) => {
    const slot = await db.get('SELECT * FROM hr_slots WHERE id = ?', slotId);
    ...
    const { n } = await db.get('SELECT COUNT(*) AS n FROM hr_bookings WHERE slot_id = ?', slotId);
    const mine = await db.get('SELECT 1 FROM hr_bookings WHERE slot_id = ? AND user_id = ?', slotId, userId);
    if (mine) return { ok: true, already: true };
    if (n >= slot.capacity) return { ok: false, error: 'that slot is full' };
    await db.run('INSERT INTO hr_bookings (slot_id, user_id, question, created_at) VALUES (?, ?, ?, ?)', slotId, userId, question, nowSeconds());
    return { ok: true, hostId: slot.host_id };
  });
}

export async function cancelBooking(slotId, userId) {
  await sql.run('DELETE FROM hr_bookings WHERE slot_id = ? AND user_id = ?', slotId, userId);
}

export async function cancelSlot(slotId) {
  await sql.run('UPDATE hr_slots SET canceled = 1 WHERE id = ?', slotId);
}

export async function slotBookings(slotId) {
  return await sql.all('SELECT * FROM hr_bookings WHERE slot_id = ? ORDER BY created_at', slotId);
}

export async function myBookings(userId) {
  return await sql.all(`SELECT s.*, b.question, b.created_at AS booked_at FROM hr_bookings b
       JOIN hr_slots s ON s.id = b.slot_id
       WHERE b.user_id = ? AND s.canceled = 0 ORDER BY s.starts_at ASC`, userId);
}
...
export async function createJob({ orgId, postedBy, title, discipline = 'other', employment = 'full-time',
  ...
  const info = await sql.run(`INSERT INTO hr_jobs (org_id, posted_by, title, discipline, employment, location, remote, comp, equity, description, tags, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, orgId, postedBy, title, discipline, employment, location, int(remote), comp, equity, description, tags, nowSeconds());
  return info.id;
}

export async function getJob(id) {
  return await sql.get(`SELECT j.*, o.name AS org_name, o.slug AS org_slug, o.kind AS org_kind
       FROM hr_jobs j JOIN hr_orgs o ON o.id = j.org_id WHERE j.id = ?`, Number(id)) ?? null;
}
...
export async function listJobs({ q = '', discipline = '', remote = false, orgId = 0, limit = 60, offset = 0 } = {}) {
  ...
  const clause = where.join(' AND ');
  const jobs = await sql.all(`SELECT j.*, o.name AS org_name, o.slug AS org_slug,
              (SELECT COUNT(*) FROM hr_applications a WHERE a.job_id = j.id) AS applicant_count
       FROM hr_jobs j JOIN hr_orgs o ON o.id = j.org_id
       WHERE ${clause} ORDER BY j.created_at DESC LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_jobs j JOIN hr_orgs o ON o.id = j.org_id WHERE ${clause}`, ...params);
  return { jobs, total };
}

export async function applyToJob(jobId, userId, note = '') {
  await sql.run('INSERT INTO hr_applications (job_id, user_id, note, created_at) VALUES (?, ?, ?, ?) ON CONFLICT DO NOTHING', jobId, userId, note, nowSeconds());
}

export async function hasApplied(jobId, userId) {
  if (!userId) return false;
  return !!await sql.get('SELECT 1 FROM hr_applications WHERE job_id = ? AND user_id = ?', jobId, userId);
}

export async function jobApplicants(jobId) {
  return await sql.all(`SELECT a.*, m.name, m.headline FROM hr_applications a
...
export async function closeJob(id, closed = true) {
  await sql.run('UPDATE hr_jobs SET closed = ? WHERE id = ?', int(closed), id);
}
...
export async function createEvent({ hostId, title, description = '', kind = 'meetup', startsAt,
  ...
  const info = await sql.run(`INSERT INTO hr_events (host_id, title, description, kind, starts_at, minutes, place, url, capacity, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, hostId, title, description, kind, startsAt, minutes, place, url, capacity, nowSeconds());
  return info.id;
}

export async function getEvent(id) {
  const row = await sql.get('SELECT * FROM hr_events WHERE id = ?', Number(id));
  if (!row) return null;
  row.going = (await sql.get("SELECT COUNT(*) AS n FROM hr_rsvps WHERE event_id = ? AND status = 'going'", row.id)).n;
  return row;
}
...
export async function listEvents({ upcoming = true, kind = '', limit = 60 } = {}) {
  ...
  if (kind) { where.push('kind = ?'); params.push(kind); }
  return await sql.all(`SELECT e.*, (SELECT COUNT(*) FROM hr_rsvps r WHERE r.event_id = e.id AND r.status = 'going') AS going
       FROM hr_events e WHERE ${where.join(' AND ')}
       ORDER BY e.starts_at ${upcoming ? 'ASC' : 'DESC'} LIMIT ?`, ...params, limit);
}

export async function rsvp(eventId, userId, status = 'going') {
  const now = nowSeconds();
  if (status === 'none') {
    await sql.run('DELETE FROM hr_rsvps WHERE event_id = ? AND user_id = ?', eventId, userId);
    return;
  }
  await sql.run(`INSERT INTO hr_rsvps (event_id, user_id, status, created_at) VALUES (?, ?, ?, ?)
       ON CONFLICT (event_id, user_id) DO UPDATE SET status = excluded.status`, eventId, userId, status, now);
}

export async function myRsvp(eventId, userId) {
  if (!userId) return null;
  return (await sql.get('SELECT status FROM hr_rsvps WHERE event_id = ? AND user_id = ?', eventId, userId))?.status ?? null;
}

export async function eventAttendees(eventId) {
  return await sql.all(`SELECT r.user_id, r.status, m.name, m.headline FROM hr_rsvps r
...
export async function cancelEvent(id) {
  await sql.run('UPDATE hr_events SET canceled = 1 WHERE id = ?', id);
}
...
export async function createLibraryEntry({ title, kind = 'guide', summary = '', body = '', tags = '', authorId = null }) {
  ...
  const info = await sql.run(`INSERT INTO hr_library (slug, title, kind, summary, body, tags, author_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, title, kind, summary, body, tags, authorId, now, now);
  return info.id;
}
...
export async function getLibraryEntry(idOrSlug) {
  const db = sql;
  return (/^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_library WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_library WHERE slug = ?', String(idOrSlug))) ?? null;
}
...
export async function listLibrary({ q = '', kind = '', limit = 60, offset = 0 } = {}) {
  ...
  if (kind) { where.push('kind = ?'); params.push(kind); }
  const clause = where.join(' AND ');
  const entries = await sql.all(`SELECT * FROM hr_library WHERE ${clause} ORDER BY updated_at DESC LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_library WHERE ${clause}`, ...params);
  return { entries, total };
}

export async function bumpReads(id) {
  await sql.run('UPDATE hr_library SET reads = reads + 1 WHERE id = ?', id);
}

/* ----------------------------------------------------------------- intros */

export async function requestIntro({ requesterId, targetId, reason }) {
  const existing = await sql.get("SELECT id FROM hr_intros WHERE requester_id = ? AND target_id = ? AND status = 'pending'", requesterId, targetId);
  if (existing) return { ok: false, error: 'you already have a pending request to this member', id: existing.id };
  const info = await sql.run('INSERT INTO hr_intros (requester_id, target_id, reason, created_at) VALUES (?, ?, ?, ?)', requesterId, targetId, reason, nowSeconds());
  return { ok: true, id: info.id };
}

export async function getIntro(id) {
  return await sql.get('SELECT * FROM hr_intros WHERE id = ?', Number(id)) ?? null;
}
...
export async function resolveIntro(id, status) {
  ...
  await sql.run('UPDATE hr_intros SET status = ?, resolved_at = ? WHERE id = ?', status, nowSeconds(), id);
...
export async function introsFor(userId) {
  const db = sql;
  return {
    incoming: await db.all('SELECT * FROM hr_intros WHERE target_id = ? ORDER BY created_at DESC LIMIT 50', userId),
    outgoing: await db.all('SELECT * FROM hr_intros WHERE requester_id = ? ORDER BY created_at DESC LIMIT 50', userId),
  };
}

export async function pendingIntroCount(userId) {
  return (await sql.get("SELECT COUNT(*) AS n FROM hr_intros WHERE target_id = ? AND status = 'pending'", userId)).n;
}
...
export async function createThread({ createdBy, subject = '', memberIds = [] }) {
  const now = nowSeconds();
  return await sql.tx(async (db) => {
    const info = await db.run('INSERT INTO hr_threads (subject, created_by, created_at, last_at) VALUES (?, ?, ?, ?)', subject, createdBy, now, now);
    const id = info.id;
    for (const member of new Set([createdBy, ...memberIds])) {
      await db.run(`INSERT INTO hr_thread_members (thread_id, user_id, last_read_at) VALUES (?, ?, ?)
                    ON CONFLICT DO NOTHING`, id, member, member === createdBy ? now : 0);
    }
    return id;
  });
}
...
export async function findDirectThread(a, b) {
  return (await sql.get(`SELECT t.id FROM hr_threads t
       ...
       ORDER BY t.id LIMIT 1`, a, b))?.id ?? null;
}

export async function openDirectThread(a, b) {
  return (await findDirectThread(a, b))
    ?? (await createThread({ createdBy: a, subject: '', memberIds: [b] }));
}

export async function sendMessage({ threadId, senderId, body }) {
  const now = nowSeconds();
  return await sql.tx(async (db) => {
    const member = await db.get('SELECT 1 FROM hr_thread_members WHERE thread_id = ? AND user_id = ?', threadId, senderId);
    if (!member) throw new Error('not a member of this thread');
    const info = await db.run('INSERT INTO hr_messages (thread_id, sender_id, body, created_at) VALUES (?, ?, ?, ?)', threadId, senderId, body, now);
    await db.run('UPDATE hr_threads SET last_at = ? WHERE id = ?', now, threadId);
    await db.run('UPDATE hr_thread_members SET last_read_at = ? WHERE thread_id = ? AND user_id = ?', now, threadId, senderId);
    return info.id;
  });
}

export async function threadsFor(userId) {
  const rows = await sql.all(`SELECT t.*, tm.last_read_at,
       ...
       WHERE tm.user_id = ? ORDER BY t.last_at DESC LIMIT 100`, userId, userId);
  for (const row of rows) row.members = await threadMembers(row.id);
  return rows;
}

export async function threadMembers(threadId) {
  return (await sql.all('SELECT user_id FROM hr_thread_members WHERE thread_id = ?', threadId))
    .map((r) => r.user_id);
}

export async function getThread(threadId, userId) {
  const thread = await sql.get('SELECT * FROM hr_threads WHERE id = ?', Number(threadId));
  if (!thread) return null;
  const members = await threadMembers(thread.id);
  if (userId && !members.includes(userId)) return null;
  thread.members = members;
  thread.messages = await sql.all('SELECT * FROM hr_messages WHERE thread_id = ? ORDER BY created_at ASC LIMIT 500', thread.id);
  return thread;
}

export async function markThreadRead(threadId, userId) {
  await sql.run('UPDATE hr_thread_members SET last_read_at = ? WHERE thread_id = ? AND user_id = ?', nowSeconds(), threadId, userId);
}

export async function unreadMessageCount(userId) {
  return (await sql.get(`SELECT COUNT(*) AS n FROM hr_messages m
       JOIN hr_thread_members tm ON tm.thread_id = m.thread_id AND tm.user_id = ?
       WHERE m.created_at > tm.last_read_at AND m.sender_id <> ?`, userId, userId)).n;
}
...
export async function notify({ userId, kind, actorId = null, text, href = '/homeroom' }) {
  ...
  const info = await sql.run('INSERT INTO hr_notifications (user_id, kind, actor_id, text, href, created_at) VALUES (?, ?, ?, ?, ?, ?)', userId, kind, actorId, text, href, nowSeconds());
  return info.id;
}
...
export async function notifications(userId, { limit = 50 } = {}) {
  return await sql.all('SELECT * FROM hr_notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', userId, limit);
}

export async function unreadNotificationCount(userId) {
  return (await sql.get('SELECT COUNT(*) AS n FROM hr_notifications WHERE user_id = ? AND read_at IS NULL', userId)).n;
}

export async function markNotificationsRead(userId) {
  await sql.run('UPDATE hr_notifications SET read_at = ? WHERE user_id = ? AND read_at IS NULL', nowSeconds(), userId);
}
...
export async function networkStats() {
  const db = sql;
  const one = async (sql, ...params) => await db.get(sql, ...params);
  return {
    members: (await one('SELECT COUNT(*) AS n FROM hr_members')).n,
    orgs: (await one('SELECT COUNT(*) AS n FROM hr_orgs')).n,
    deals: (await one('SELECT COUNT(*) AS n FROM hr_deals WHERE active = 1')).n,
    funders: (await one('SELECT COUNT(*) AS n FROM hr_funders')).n,
    reviews: (await one('SELECT COUNT(*) AS n FROM hr_funder_reviews')).n,
    jobs: (await one('SELECT COUNT(*) AS n FROM hr_jobs WHERE closed = 0')).n,
    slots: await one('SELECT COUNT(*) AS n FROM hr_slots WHERE canceled = 0 AND starts_at > ?', nowSeconds()).n,
    events: await one('SELECT COUNT(*) AS n FROM hr_events WHERE canceled = 0 AND starts_at > ?', nowSeconds()).n,
    library: (await one('SELECT COUNT(*) AS n FROM hr_library')).n,
    mentors: (await one('SELECT COUNT(*) AS n FROM hr_mentors WHERE active = 1')).n,
    vetted: (await one('SELECT COUNT(*) AS n FROM hr_mentors WHERE active = 1 AND vetted = 1')).n,
    atlas: (await one('SELECT COUNT(*) AS n FROM hr_atlas')).n,
    atlasActive: await one(`SELECT COUNT(*) AS n FROM hr_atlas WHERE status = 'active'`).n,
    modules: (await one('SELECT COUNT(*) AS n FROM hr_modules')).n,
    yearbook: (await one('SELECT COUNT(*) AS n FROM hr_yearbook')).n,
  };
}
...
export async function onboardingSteps(userId) {
  ...
  const one = async (sql, ...params) => (await db.get(sql, ...params))?.n || 0;
  ...
  return [
    ...
    },
    {
      ...
      done: await one('SELECT COUNT(*) AS n FROM hr_yearbook WHERE user_id = ?', userId) > 0,
    },
    {
      key: 'perk',
      title: 'Claim a perk',
      ...
      done: await one('SELECT COUNT(*) AS n FROM hr_deal_claims WHERE user_id = ?', userId) > 0,
    },
    {
      key: 'manual',
      ...
      done: await one('SELECT COUNT(*) AS n FROM hr_progress WHERE user_id = ?', userId) > 0,
    },
    {
      key: 'hours',
      ...
      done: await one('SELECT COUNT(*) AS n FROM hr_bookings WHERE user_id = ?', userId) > 0,
    },
    {
      key: 'lab',
      title: 'Add your lab',
      ...
      action: 'Add a lab',
      optional: true,
      done: await one('SELECT COUNT(*) AS n FROM hr_org_members WHERE user_id = ?', userId) > 0,
    },
  ];
}
...
export async function getYearbook(userId) {
  return await sql.get('SELECT * FROM hr_yearbook WHERE user_id = ?', userId) ?? null;
}
...
export async function upsertYearbook(userId, patch = {}) {
  ...
  await sql.run(`INSERT INTO hr_yearbook (user_id, cohort, house, venture, one_liner, quote, building,
       ...
       ON CONFLICT (user_id) DO UPDATE SET
         ...
         site_url = excluded.site_url, featured = excluded.featured, updated_at = excluded.updated_at`, userId, value('cohort'), value('house'), value('venture'), value('one_liner'),
...
export async function yearbookWall({ cohort = '', house = '', q = '', tag = '',
  ...
  const clause = where.join(' AND ');
  const rows = await sql.all(`SELECT m.*, u.karma,
       ...
       LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_members m
       LEFT JOIN hr_yearbook y ON y.user_id = m.user_id WHERE ${clause}`, ...params);
...
export async function wallCohorts() {
  return await sql.all(`SELECT cohort, COUNT(*) AS n FROM (
...
export async function houses() {
  return await sql.all(`SELECT house, COUNT(*) AS n FROM hr_yearbook WHERE house <> '' GROUP BY house ORDER BY house`);
}

export async function signatures(userId) {
  return await sql.all('SELECT * FROM hr_signatures WHERE user_id = ? ORDER BY created_at DESC', userId);
}
...
export async function signYearbook({ userId, authorId, body }) {
  ...
  await sql.run(`INSERT INTO hr_signatures (user_id, author_id, body, created_at) VALUES (?, ?, ?, ?)
       ON CONFLICT (user_id, author_id) DO UPDATE SET body = excluded.body, created_at = excluded.created_at`, userId, authorId, text, nowSeconds());
  return { ok: true };
}
...
export async function upsertLab(lab) {
  ...
  const existing = await sql.get('SELECT id FROM hr_atlas WHERE slug = ?', slug);
  const capabilities = Array.isArray(lab.capabilities) ? lab.capabilities.join(',') : (lab.capabilities || '');
  if (existing) {
    await sql.run(`UPDATE hr_atlas SET name = ?, city = ?, country = ?, region = ?, kind = ?, status = ?,
                bsl = ?, website = ?, capabilities = ?, note = ?, source = ? WHERE id = ?`, lab.name, lab.city, lab.country, lab.region, lab.kind, lab.status,
        lab.bsl || '', lab.website || null, capabilities, lab.note || '', lab.source || '', existing.id);
    return existing.id;
  }
  const info = await sql.run(`INSERT INTO hr_atlas (slug, name, city, country, region, kind, status, bsl, website,
                             capabilities, note, source, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, lab.name, lab.city, lab.country, lab.region, lab.kind, lab.status,
...
export async function getLab(idOrSlug) {
  const db = sql;
  const row = (/^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_atlas WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_atlas WHERE slug = ?', String(idOrSlug))) ?? null;
  if (!row) return null;
...
export async function searchLabs({ q = '', region = '', country = '', status = '', kind = '',
  ...
  const clause = where.join(' AND ');
  const rows = await sql.all(`SELECT *, (SELECT COUNT(*) FROM hr_atlas_reports r WHERE r.lab_id = hr_atlas.id) AS reports
       FROM hr_atlas WHERE ${clause}
       ORDER BY CASE status WHEN 'active' THEN 0 WHEN 'limited' THEN 1 WHEN 'unknown' THEN 2 ELSE 3 END,
                country, city, name
       LIMIT ? OFFSET ?`, ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_atlas WHERE ${clause}`, ...params);
  return {
...
export async function atlasFacets() {
  const db = sql;
  return {
    regions: await db.all(`SELECT region AS slug, region AS label, COUNT(*) AS count FROM hr_atlas
                         WHERE region <> '' GROUP BY region ORDER BY count DESC`),
    countries: await db.all(`SELECT country AS slug, country AS label, COUNT(*) AS count FROM hr_atlas
                           WHERE country <> '' GROUP BY country ORDER BY country`),
    statuses: await db.all(`SELECT status AS slug, status AS label, COUNT(*) AS count FROM hr_atlas
                          GROUP BY status`),
  };
}
...
export async function reportLab({ labId, userId, status, body = '' }) {
  ...
  return await sql.tx(async (db) => {
    const now = nowSeconds();
    await db.run('INSERT INTO hr_atlas_reports (lab_id, user_id, status, body, created_at) VALUES (?, ?, ?, ?, ?)', labId, userId, status, String(body || '').slice(0, 2000), now);
    await db.run('UPDATE hr_atlas SET status = ?, confirmed_by = ?, confirmed_at = ? WHERE id = ?', status, userId, now, labId);
    return { ok: true };
  });
}

export async function labReports(labId) {
  return await sql.all('SELECT * FROM hr_atlas_reports WHERE lab_id = ? ORDER BY created_at DESC LIMIT 20', labId);
}
...
export async function upsertMentor(mentor) {
  ...
  const existing = await sql.get('SELECT id FROM hr_mentors WHERE slug = ?', slug);
  if (existing) {
    await sql.run(`UPDATE hr_mentors SET name = ?, role = ?, org = ?, track = ?, tags = ?, location = ?,
                bio = ?, format = ?, scheduler = ?, vetted = ?, user_id = ?, source = ? WHERE id = ?`, mentor.name, mentor.role || '', mentor.org || '', mentor.track || 'founder', tags,
  ...
  const info = await sql.run(`INSERT INTO hr_mentors (slug, user_id, name, role, org, track, tags, location, bio,
                               format, scheduler, vetted, source, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, mentor.userId || null, mentor.name, mentor.role || '', mentor.org || '',
...
export async function getMentor(idOrSlug) {
  const db = sql;
  const row = (/^\d+$/.test(String(idOrSlug))
    ? await db.get(`SELECT ${MENTOR_COLUMNS} FROM hr_mentors WHERE id = ?`, Number(idOrSlug))
    : await db.get(`SELECT ${MENTOR_COLUMNS} FROM hr_mentors WHERE slug = ?`, String(idOrSlug))) ?? null;
  if (!row) return null;
...
export async function searchMentors({ q = '', track = '', tag = '', vetted = false, format = '',
  ...
  const clause = where.join(' AND ');
  const rows = await sql.all(`SELECT ${MENTOR_COLUMNS_M},
       ...
       ORDER BY m.vetted DESC, open_slots DESC, lower(m.name) LIMIT ? OFFSET ?`, nowSeconds(), ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_mentors WHERE ${clause}`, ...params);
  return { mentors: rows.map((row) => ({ ...row, tags: tagList(row.tags) })), total };
}

export async function mentorTagCloud(limit = 40) {
  const rows = await sql.all('SELECT tags FROM hr_mentors WHERE active = 1');
...
export async function mentorSlots(mentorId) {
  return await sql.all(`SELECT s.*, (SELECT COUNT(*) FROM hr_bookings b WHERE b.slot_id = s.id) AS booked
       ...
       ORDER BY s.starts_at`, mentorId, nowSeconds());
}

export async function bumpMentorSessions(mentorId) {
  await sql.run('UPDATE hr_mentors SET sessions = sessions + 1 WHERE id = ?', mentorId);
}
...
export async function getReview(id) {
  return await sql.get('SELECT * FROM hr_funder_reviews WHERE id = ?', id) ?? null;
}
...
export async function addReviewComment({ reviewId, authorId, body, anonymous = false }) {
  ...
  const info = await sql.run('INSERT INTO hr_review_comments (review_id, author_id, body, anonymous, created_at) VALUES (?, ?, ?, ?, ?)', reviewId, authorId, text.slice(0, 4000), int(anonymous), nowSeconds());
  return { ok: true, id: info.id };
}

export async function reviewComments(reviewIds) {
  if (!reviewIds.length) return {};
  const rows = await sql.all(`SELECT * FROM hr_review_comments
       WHERE review_id IN (${placeholders(reviewIds.length)}) AND deleted = 0
       ORDER BY created_at`, ...reviewIds);
  const out = {};
...
export async function deleteReviewComment(id, userId, { isAdmin = false } = {}) {
  const row = await sql.get('SELECT * FROM hr_review_comments WHERE id = ?', id);
  if (!row || (row.author_id !== userId && !isAdmin)) return false;
  await sql.run('UPDATE hr_review_comments SET deleted = 1 WHERE id = ?', id);
  return true;
}
...
export async function toggleReviewHelpful(reviewId, userId) {
  const db = sql;
  const existing = await db.get('SELECT 1 FROM hr_review_votes WHERE review_id = ? AND user_id = ?', reviewId, userId);
  if (existing) {
    await db.run('DELETE FROM hr_review_votes WHERE review_id = ? AND user_id = ?', reviewId, userId);
    return false;
  }
  await db.run('INSERT INTO hr_review_votes (review_id, user_id, helpful, created_at) VALUES (?, ?, 1, ?)', reviewId, userId, nowSeconds());
  return true;
}

export async function helpfulIds(userId, reviewIds) {
  if (!userId || !reviewIds.length) return new Set();
  const rows = await sql.all(`SELECT review_id FROM hr_review_votes WHERE user_id = ? AND review_id IN (${placeholders(reviewIds.length)})`, userId, ...reviewIds);
...
export async function funderTagCloud(funderId, limit = 8) {
  const rows = await sql.all('SELECT tags FROM hr_funder_reviews WHERE funder_id = ?', funderId);
  const counts = new Map();
  for (const row of rows) {
    for (const tag of tagList(row.tags)) counts.set(tag, (counts.get(tag) || 0) + 1);
  }
...
export async function upsertTrack(track, position = 0) {
  await sql.run(`INSERT INTO hr_tracks (slug, title, focus, blurb, position) VALUES (?, ?, ?, ?, ?)
       ON CONFLICT (slug) DO UPDATE SET title = excluded.title, focus = excluded.focus,
         blurb = excluded.blurb, position = excluded.position`, track.slug, track.title, track.focus || '', track.blurb || '', position);
}

export async function upsertModule(module, position = 0) {
  await sql.run(`INSERT INTO hr_modules (slug, track, title, kind, summary, outcomes, work, deliverable,
       ...
       ON CONFLICT (slug) DO UPDATE SET track = excluded.track, title = excluded.title,
         ...
         week = excluded.week, position = excluded.position`, module.slug, module.track, module.title, module.kind, module.summary,
...
export async function tracks() {
  return await sql.all('SELECT * FROM hr_tracks ORDER BY position, title');
}

export async function getTrack(slug) {
  return await sql.get('SELECT * FROM hr_tracks WHERE slug = ?', slug) ?? null;
}

export async function getModule(idOrSlug) {
  const db = sql;
  return hydrateModule(/^\d+$/.test(String(idOrSlug))
    ? await db.get('SELECT * FROM hr_modules WHERE id = ?', Number(idOrSlug))
    : await db.get('SELECT * FROM hr_modules WHERE slug = ?', String(idOrSlug)));
}
...
export async function listModules({ track = '', kind = '', q = '', week = 0, userId = '',
  ...
  const clause = where.join(' AND ');
  const rows = await sql.all(`SELECT m.*, t.title AS track_title,
       ...
       WHERE ${clause} ORDER BY t.position, m.position LIMIT ? OFFSET ?`, userId || '', ...params, limit, offset);
  const { total } = await sql.get(`SELECT COUNT(*) AS total FROM hr_modules m WHERE ${clause}`, ...params);
  return { modules: rows.map(hydrateModule), total };
}

export async function setProgress({ userId, moduleId, state = 'started', note = '', link = '' }) {
  if (state === 'none') {
    await sql.run('DELETE FROM hr_progress WHERE user_id = ? AND module_id = ?', userId, moduleId);
    return null;
  }
  await sql.run(`INSERT INTO hr_progress (user_id, module_id, state, note, link, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)
       ON CONFLICT (user_id, module_id) DO UPDATE SET state = excluded.state,
         note = excluded.note, link = excluded.link, updated_at = excluded.updated_at`, userId, moduleId, state, String(note || '').slice(0, 4000), String(link || '').slice(0, 500), nowSeconds());
  return await getProgress(userId, moduleId);
}

export async function getProgress(userId, moduleId) {
  if (!userId) return null;
  return await sql.get('SELECT * FROM hr_progress WHERE user_id = ? AND module_id = ?', userId, moduleId) ?? null;
}
...
export async function progressSummary(userId) {
  const db = sql;
  const total = (await db.get('SELECT COUNT(*) AS n FROM hr_modules')).n;
  const rows = await db.all(`SELECT m.track, t.title AS track_title, COUNT(m.id) AS total,
       ...
       GROUP BY m.track ORDER BY t.position`, userId || '');
...
export async function deliverables(userId) {
  return await sql.all(`SELECT m.slug, m.title, m.deliverable, m.week, p.state, p.note, p.link, p.updated_at
       FROM hr_progress p JOIN hr_modules m ON m.id = p.module_id
       WHERE p.user_id = ? AND m.deliverable <> '' ORDER BY m.week, m.position`, userId);
}

export async function bumpModuleReads(id) {
  await sql.run('UPDATE hr_modules SET reads = reads + 1 WHERE id = ?', id);
}
...
export async function eventsBetween(startsAt, endsAt) {
  return await sql.all(`SELECT e.*,
       ...
       WHERE e.starts_at >= ? AND e.starts_at < ? ORDER BY e.starts_at`, startsAt, endsAt);
}

export async function eventSource(eventId) {
  return await sql.get('SELECT * FROM hr_event_sources WHERE event_id = ?', eventId) ?? null;
}
...
export async function upsertExternalEvent({ source = 'luma', externalId, hostId, title, description = '',
  ...
  return await sql.tx(async (db) => {
    const now = nowSeconds();
    const existing = await db.get('SELECT event_id FROM hr_event_sources WHERE source = ? AND external_id = ?', source, externalId);
    if (existing) {
      await db.run(`UPDATE hr_events SET title = ?, description = ?, kind = ?, starts_at = ?, minutes = ?,
                place = ?, url = ?, capacity = ?, canceled = ? WHERE id = ?`, title, description, kind, startsAt, minutes, place, url, capacity, int(canceled), existing.event_id);
      await db.run('UPDATE hr_event_sources SET url = ?, synced_at = ? WHERE event_id = ?', url || '', now, existing.event_id);
      return { id: existing.event_id, created: false };
    }
    const info = await db.run(`INSERT INTO hr_events (host_id, title, description, kind, starts_at, minutes, place, url,
                                capacity, canceled, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, hostId, title, description, kind, startsAt, minutes, place, url, capacity, int(canceled), now);
    const id = info.id;
    await db.run('INSERT INTO hr_event_sources (event_id, source, external_id, url, synced_at) VALUES (?, ?, ?, ?, ?)', id, source, externalId, url || '', now);
    return { id, created: true };
  });
}

export async function lastSync(source = 'luma') {
  return await sql.get('SELECT MAX(synced_at) AS at, COUNT(*) AS n FROM hr_event_sources WHERE source = ?', source);
}
...
export async function recordNewsSubmission({ userId, title, url = '', body = '', topic = 'general',
  ...
  const now = nowSeconds();
  const info = await sql.run(`INSERT INTO hr_news_submissions (user_id, remote_id, title, url, body, topic, status, error, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, userId, remoteId, title, url, body, topic, status, error, now, now);
  return info.id;
}

export async function updateNewsSubmission(id, { status, remoteId, error = '' }) {
  await sql.run('UPDATE hr_news_submissions SET status = ?, remote_id = COALESCE(?, remote_id), error = ?, updated_at = ? WHERE id = ?', status, remoteId ?? null, error, nowSeconds(), id);
}

export async function newsSubmissions(userId, { limit = 30 } = {}) {
  return await sql.all('SELECT * FROM hr_news_submissions WHERE user_id = ? ORDER BY created_at DESC LIMIT ?', userId, limit);
}
...
export async function recordVerdict({ hash, masked, verdict, reason, person = {} }) {
  const now = nowSeconds();
  await sql.run(`INSERT INTO hr_roster (email_hash, masked, verdict, reason, name, cohort, house,
       ...
       ON CONFLICT (email_hash) DO UPDATE SET
         ...
         attempts = hr_roster.attempts + 1, checked_at = excluded.checked_at`, hash, masked, verdict, reason, person.name || '', person.cohort || '', person.house || '',
      person.status || '', person.lifecycle || '', person.residentType || '', now);
  return await rosterRow(hash);
}

export async function rosterRow(hash) {
  return await sql.get('SELECT * FROM hr_roster WHERE email_hash = ?', hash) ?? null;
}

export async function linkRosterUser(hash, userId) {
  await sql.run('UPDATE hr_roster SET user_id = ? WHERE email_hash = ?', userId, hash);
}

export async function setUserRoster(userId, status) {
  await sql.run('UPDATE users SET roster_status = ?, roster_checked_at = ? WHERE id = ?', status, nowSeconds(), userId);
}
...
export async function decideRoster({ hash, userId, decision, note = '' }) {
  await sql.run('UPDATE hr_roster SET decision = ?, decided_by = ?, decided_at = ?, note = ? WHERE email_hash = ?', decision, userId, nowSeconds(), String(note || '').slice(0, 500), hash);
  return await rosterRow(hash);
}

/** Conflicts a steward has not ruled on yet. This is the queue that matters. */
export async function pendingRoster() {
  return await sql.all(`SELECT * FROM hr_roster WHERE verdict = 'review' AND decision IS NULL
              ORDER BY checked_at DESC LIMIT 100`);
}

export async function recentRoster({ limit = 60 } = {}) {
  return await sql.all('SELECT * FROM hr_roster ORDER BY checked_at DESC LIMIT ?', limit);
}

export async function rosterCounts() {
  const rows = await sql.all('SELECT verdict, COUNT(*) AS n FROM hr_roster GROUP BY verdict');
...
export async function upsertDeal(deal) {
  ...
  const db = sql;
  const existing = await db.get('SELECT id FROM hr_deals WHERE slug = ?', slug);
  if (existing) {
    await db.run(`UPDATE hr_deals SET vendor = ?, title = ?, category = ?, summary = ?, details = ?,
              worth = ?, url = ?, access = ?, requirement = ?, checked = ? WHERE id = ?`, deal.vendor, deal.title, deal.category, deal.summary || '', deal.details || '',
  ...
  const info = await db.run(`INSERT INTO hr_deals (slug, vendor, title, category, summary, details, worth, code, url,
                             posted_by, created_at, access, requirement, checked)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, deal.vendor, deal.title, deal.category, deal.summary || '', deal.details || '',
      deal.worth || '', deal.code || '', deal.url || null, deal.postedBy, nowSeconds(),
      deal.access || 'code', deal.requirement || '', deal.checked || '');
  return { id: info.id, slug, created: true };
}

export async function upsertFunder(funder) {
  const slug = slugify(funder.name, 'funder');
  const db = sql;
  const existing = await db.get('SELECT id FROM hr_funders WHERE slug = ?', slug);
  if (existing) {
    await db.run(`UPDATE hr_funders SET name = ?, kind = ?, focus = ?, stages = ?, check_size = ?,
              location = ?, website = ?, description = ?, dilutive = ? WHERE id = ?`, funder.name, funder.kind, funder.focus || '', funder.stages || '',
      funder.checkSize || '', funder.location || '', funder.website || null,
      funder.description || '', int(funder.dilutive), existing.id);
    return { id: existing.id, slug, created: false };
  }
  const info = await db.run(`INSERT INTO hr_funders (slug, name, kind, focus, stages, check_size, location, website,
                               description, dilutive, added_by, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, slug, funder.name, funder.kind, funder.focus || '', funder.stages || '',
      funder.checkSize || '', funder.location || '', funder.website || null,
...
export async function pruneBySlug(table, keep) {
  ...
  const rows = await sql.all(`SELECT slug FROM ${table}`);
  const stale = rows.map((r) => r.slug).filter((slug) => !keep.includes(slug));
  if (!stale.length) return 0;
  for (const slug of stale) await sql.run(`DELETE FROM ${table} WHERE slug = ?`, slug);
  return stale.length;
}
