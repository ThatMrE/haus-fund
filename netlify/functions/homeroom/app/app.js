...
import * as sqlLayer from './sql.js';
...
export async function handle(req, res) {
  ...
  try {
    ...
    const handler = await homeroomRoute(req.method, pathname);
    if (!handler) return await homeroomNotFound(ctx);
    await handler(ctx);
  } catch (err) {
    if (err?.status === 413) return sendText(res, 'Payload too large.', { status: 413 });
    console.error(`[homeroom] ${req.method} ${req.url}`, err);
    if (!res.headersSent) {
      return await render(ctx, views.errorPage('Something went wrong on our side.'), {
...
const AUTH_ROUTES = {
  ...
  'POST /homeroom/login': async (ctx) => {
    ...
    if (sbAuth.configured()) {
      ...
      if (created) await access.seedProfile(linked.id, {});
