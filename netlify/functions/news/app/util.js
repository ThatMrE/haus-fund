...
/**
 * The app can be mounted under a prefix (haus.fund/news) or at a domain root.
 * Everything internal goes through `u()` so a move is one env var, not a
...
export const BASE = normalizeBase(process.env.NEWS_BASE_PATH);
export const STATIC_BASE = normalizeBase(process.env.NEWS_STATIC_BASE) || '/static';

/** Prefix an app path with the mount point. `u('/')` is the front page. */
...
/** Prefix a static asset path. */
export function s(path) {
  return STATIC_BASE + (path.startsWith('/') ? path : `/${path}`);
}

/** Strip the mount prefix off an incoming request path. */
