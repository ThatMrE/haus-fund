...
| `netlify/functions/homeroom/` | **Homeroom** — the members-only network, served at `/homeroom`, `noindex`. Forum, member and lab directories, deals, funder reviews, pipeline, office hours, jobs, events, library, intros (in the house, and out of it through the Happenstance-backed intro desk), messaging. A server-rendered app like `/news`, with its own accounts; see its README |
